...
import { listDeveloper } from '../../services/actions/developer'
...
export default function DeveloperIndex({ navigation }: any) {
  const developers = useSelector(
    (state: any) => state?.developer?.list?.data || []
  )
  const Dispatch = useDispatch()
  useEffect(() => {
    Dispatch(listDeveloper())
  }, [])
  ...
  return (
    <SafeAreaView>
      <Appbar.Header style={style.header}>
        ...
        />
        <Appbar.Content title="Developer" />
      </Appbar.Header>
      <View style={style.wrapper}>
        <View style={style.scrolable}>
          <ScrollView>
            {developers.map((usr: any, idx: number) => (
              <List.Item
                title={`${_.startCase(usr.name)} | ${usr.email}`}
                description={`${moment(usr.created_at).format('DD/MM/YYYY')}`}
                key={idx}
                style={style.developerItem}
                right={(props) => (
                  <IconButton
                    size={24}
      ...
      <View style={style.absoluteAdd}>
        <Button
          icon="plus"
          mode="contained"
          onPress={() => navigation.navigate('developer.create')}
