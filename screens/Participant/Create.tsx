...
import { createParticipant } from '../../services/actions/participant'
...
export default function ParticipantCreate({ navigation }: any) {
  ...
  const isValid = useMemo(() => {
    return Object.entries(form).filter((c: any) => c[1]?.length).length >= 3
  }, [form])
  ...
  return (
    <SafeAreaView>
      <Appbar.Header style={style.header}>
        ...
        />
        <Appbar.Content title="Create New Participant" />
      </Appbar.Header>
      ...
      <View style={style.wrapper}>
        ...
        />
        {createStatus.data?.phone?.[0] && (
          <Text style={style.error}>{createStatus.data?.phone?.[0]}</Text>
        )}
